...
async function ensureRolePermissionsReady(): Promise<void> {
  ...
  rolePermsReady = (async () => {
    ...
    // 4. Migration : ouvrir /formations-individuelles au directeur des études
...
app.post('/admin/email-mismatches/sync', requireAuth, role('dg'), async (c) => {
  ...
  // Helper interne : retrouve un user à rattacher via email, puis fallback nom+prenom.
  async function findUserFor(role: 'etudiant' | 'enseignant', fiche: { email: string | null; nom: string | null; prenom: string | null }, linkedTable: 'etudiants' | 'enseignants'): Promise<number | null> {
    if (fiche.email) {
      const { rows } = await pool.query('SELECT id FROM users WHERE LOWER(email)=LOWER($1) LIMIT 1', [fiche.email])
      ...
      const { rows } = await pool.query(
        `SELECT u.id FROM users u
         WHERE u.role=$1
           ...
           AND NOT EXISTS (SELECT 1 FROM ${linkedTable} x WHERE x.user_id = u.id)
         ORDER BY u.id DESC LIMIT 1`,
        [role, fiche.nom, fiche.prenom]
      )
      if (rows[0]) return rows[0].id
    }
    return null
  ...
  const { rows: etuds } = await pool.query(`
    ...
    FROM etudiants e
    WHERE e.email IS NOT NULL OR e.user_id IS NULL
  `)
  for (const e of etuds) {
    if (!e.user_id) {
      const uid = await findUserFor('etudiant', e, 'etudiants')
      if (uid) {
        await pool.query('UPDATE etudiants SET user_id=$1 WHERE id=$2', [uid, e.id]).catch(() => {})
        if (e.email) {
          await pool.query('UPDATE users SET nom=$1, prenom=$2, email=$3 WHERE id=$4',
            [e.nom, e.prenom, e.email, uid]).catch(() => {})
        }
  ...
  for (const e of ensList) {
    if (!e.user_id) {
      const uid = await findUserFor('enseignant', e, 'enseignants')
      if (uid) {
        await pool.query('UPDATE enseignants SET user_id=$1 WHERE id=$2', [uid, e.id]).catch(() => {})
        if (e.email) {
          await pool.query('UPDATE users SET nom=$1, prenom=$2, email=$3 WHERE id=$4',
            [e.nom, e.prenom, e.email, uid]).catch(() => {})
        }
...
app.put('/etudiants/:id', requireAuth, role('secretariat', 'dg'), async (c) => {
  ...
  // Synchroniser le compte users (nom, prenom, email). Stratégie en cascade :
  //   1) via etudiants.user_id
  //   2) fallback par ANCIEN email d'étudiant (cas fréquent : user_id jamais posé)
  //   3) fallback par (nom, prenom, role='etudiant') non encore lié à une fiche
  // La 3e branche couvre Aïssatou & co : email modifié plusieurs fois côté fiche
  // sans jamais mettre à jour users — plus aucun email ne correspond.
  ...
  if (!linkedUserId && current[0]?.email) {
    ...
    if (uRows[0]) linkedUserId = uRows[0].id
  }
  if (!linkedUserId && b.nom && b.prenom) {
    const { rows: uRows } = await pool.query(
      `SELECT u.id FROM users u
       WHERE u.role='etudiant'
         AND LOWER(u.nom)=LOWER($1)
  ...
  if (linkedUserId) {
    await pool.query('UPDATE etudiants SET user_id=$1 WHERE id=$2 AND (user_id IS NULL OR user_id<>$1)', [linkedUserId, id]).catch(() => {})
    await pool.query(
      'UPDATE users SET nom=$1, prenom=$2, email=$3 WHERE id=$4',
...
app.post('/inscriptions', requireAuth, role('secretariat', 'dg'), async (c) => {
  ...
  // et surtout lier etudiants.user_id pour que la synchro fonctionne ensuite.
  try {
    const { rows: etud } = await pool.query('SELECT id,nom,prenom,email,user_id FROM etudiants WHERE id=$1', [b.etudiant_id])
    if (etud[0]?.email) {
...
app.put('/enseignants/:id', requireAuth, role('dg', 'dir_peda', 'secretariat'), async (c) => {
  ...
  // Synchroniser users. Stratégie en cascade identique aux étudiants :
  //   1) enseignants.user_id  2) ancien email  3) (nom, prenom, role='enseignant')
  let linkedUserId: number | null = current[0]?.user_id ?? null
  if (!linkedUserId && current[0]?.email) {
    const { rows: uRows } = await pool.query(
      'SELECT id FROM users WHERE LOWER(email)=LOWER($1) LIMIT 1',
    ...
    const { rows: uRows } = await pool.query(
      `SELECT u.id FROM users u
       WHERE u.role='enseignant'
         AND LOWER(u.nom)=LOWER($1)
         AND LOWER(u.prenom)=LOWER($2)
         AND NOT EXISTS (SELECT 1 FROM enseignants e WHERE e.user_id = u.id)
       ORDER BY u.id DESC LIMIT 1`,
      [b.nom, b.prenom]
    )
    if (uRows[0]) linkedUserId = uRows[0].id
  ...
  if (linkedUserId) {
    await pool.query('UPDATE enseignants SET user_id=$1 WHERE id=$2 AND (user_id IS NULL OR user_id<>$1)', [linkedUserId, id]).catch(() => {})
    await pool.query(
      'UPDATE users SET nom=$1, prenom=$2, email=$3 WHERE id=$4',
      [b.nom, b.prenom, b.email || null, linkedUserId]
    ).catch(() => {})
...
// Chaque requête est enveloppée pour ne jamais faire échouer la réponse — les
// valeurs manquantes remontent à 0, jamais un 500.
app.get('/dashboard/priorites', requireAuth, async (c) => {
  const today = new Date().toISOString().slice(0, 10)
  const currentMonth = new Date().toISOString().slice(0, 7)

  const safe = async <T>(q: Promise<{ rows: any[] }>, fallback: T): Promise<T> => {
    try {
      const r = await q
      return (r.rows[0] ?? fallback) as T
    } catch (e: any) {
      console.warn('[priorites] query failed:', e?.message || e)
      return fallback
    }
  }
  ...
  const [
    ...
    safe(pool.query("SELECT COUNT(*)::int AS n FROM inscriptions WHERE statut IN ('pre_inscrit','en_examen')"), { n: 0 }),
    safe(pool.query("SELECT COUNT(*)::int AS n, COALESCE(SUM(montant),0)::float AS total FROM paiements WHERE statut='en_attente'"), { n: 0, total: 0 }),
    safe(pool.query("SELECT COUNT(*)::int AS n, COALESCE(SUM(montant),0)::float AS total FROM depenses WHERE statut='en_attente'"), { n: 0, total: 0 }),
    safe(pool.query(`SELECT COALESCE(SUM(montant),0)::float AS total, COUNT(*)::int AS n
                FROM paiements WHERE statut='confirme' AND confirmed_at::date = $1`, [today]), { n: 0, total: 0 }),
    safe(pool.query(`SELECT COALESCE(SUM(montant),0)::float AS total
                FROM paiements WHERE statut='confirme'
                  AND TO_CHAR(confirmed_at,'YYYY-MM') = $1`, [currentMonth]), { total: 0 }),
    safe(pool.query(`SELECT COUNT(*)::int AS n, COALESCE(SUM(montant),0)::float AS total
                FROM echeances WHERE statut='non_paye' AND TO_CHAR(mois,'YYYY-MM') <= $1`, [currentMonth]), { n: 0, total: 0 }),
    safe(pool.query("SELECT COUNT(*)::int AS n FROM seances WHERE enseignant_id IS NULL AND statut != 'annule' AND date_debut >= NOW()"), { n: 0 }),
    safe(pool.query(`SELECT COUNT(*)::int AS n FROM seances WHERE statut NOT IN ('annule') AND date_debut::date = $1`, [today]), { n: 0 }),
    safe(pool.query(`SELECT COUNT(*)::int AS n
                FROM seances s
                WHERE s.statut = 'terminee'
                  AND s.date_fin::date <= CURRENT_DATE
                  AND NOT EXISTS (SELECT 1 FROM presences p WHERE p.seance_id = s.id)`), { n: 0 }),
    safe(pool.query(`SELECT COUNT(DISTINCT etudiant_id)::int AS n
                FROM (
    ...
    safe(pool.query(`SELECT COUNT(*)::int AS n FROM classes
                WHERE est_tronc_commun = false AND date_debut_cours IS NULL`), { n: 0 }),
  ])

  return c.json({
    pre_inscrits: (preInscrits as any).n ?? 0,
    paiements_en_attente: paiementsEnAttente as any,
    depenses_en_attente: depensesEnAttente as any,
    encaisse_aujourdhui: encaisseAujourdhui as any,
    encaisse_ce_mois: (encaisseCeMois as any).total ?? 0,
    impayes_actuels: impayesActuels as any,
    seances_sans_enseignant: (seancesSansEnseignant as any).n ?? 0,
    seances_aujourdhui: (seancesAujourdhui as any).n ?? 0,
    presences_non_validees: (presencesNonValidees as any).n ?? 0,
    etudiants_en_retard: (retardsMensualite as any).n ?? 0,
    classes_sans_date_debut: (classesSansDateDebut as any).n ?? 0,
  })
})
...
app.patch('/classes/:id/date-debut-cours', requireAuth, role('dg', 'coordinateur', 'resp_fin', 'secretariat', 'dir_peda'), async (c) => {
  const classeId = parseInt(c.req.param('id'))
