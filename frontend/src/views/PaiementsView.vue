      ...
      <table v-else class="w-full">
        <thead>
          <tr class="bg-gray-50 border-b border-gray-100">
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Étudiant</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Filière</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Mode</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide hidden lg:table-cell">Mois</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Montant</th>
        ...
        <tbody>
          <tr v-for="(p, i) in filtered" :key="p.id" class="border-b border-gray-50 hover:bg-gray-50 transition">
            <td class="px-4 py-3 font-mono text-xs text-gray-400">{{ p.numero_recu }}</td>
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" :style="{ background: avatarColor(i) }">
                  {{ initials(p.etudiant.prenom, p.etudiant.nom) }}
                </div>
                <div>
                  <p class="text-sm font-semibold text-gray-900">{{ p.etudiant.prenom }} {{ p.etudiant.nom }}</p>
                  <p class="text-xs text-gray-400">{{ p.etudiant.numero_etudiant }}</p>
                </div>
              </div>
            </td>
            <td class="px-4 py-3 text-sm text-gray-500 hidden md:table-cell">{{ p.filiere }}</td>
            <td class="px-4 py-3">
              <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold" :class="modeClass[p.mode] ?? 'bg-gray-100 text-gray-600'">
                {{ modeLabel[p.mode] ?? p.mode }}
              </span>
            </td>
            <td class="px-4 py-3 text-sm text-gray-500 hidden lg:table-cell">{{ p.mois_concerne }}</td>
            <td class="px-4 py-3 font-bold text-sm text-green-600">+{{ fmt(p.montant) }} FCFA</td>
            <td class="px-4 py-3">
              <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold" :class="statutClass[p.statut] ?? 'bg-gray-100 text-gray-500'">
          ...
          <tr v-if="!filtered.length">
            <td colspan="9" class="px-4 py-8 text-center text-sm text-gray-400">Aucun paiement trouvé</td>
          </tr>
        </tbody>
      </table>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showModal = false">
        <div class="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
          <div class="flex items-center justify-between p-5 border-b border-gray-100 sticky top-0 bg-white">
            <h2 class="text-base font-bold">Enregistrer un paiement</h2>
